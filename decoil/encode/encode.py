...
def parsevcf(vcffile_clean, multi, svcaller):
	...
	try:
		for record in reader:
			...
			state, process_svs = operations.pass_filter(record, processed_mates=process_svs, multi=multi)
			...
			# multi-sample vcf
				...
				dv_arr = [record.calls[i].data[vp.DR][1] for i in range(len(record.calls))] # it is correct to DR only
				dr_arr = [record.calls[i].data[vp.DR][0] for i in range(len(record.calls))] # it is correct to DR only
				gt_arr = [record.calls[i].data[vp.GT] for i in range(len(record.calls))]
		...
		log.warning(
			"VCF file was cropped! Most probably because VCF is corrupted. This will generate incomplete/incorrect reconstruction."
		)
		sys.exit(1)
	except Exception:
		print(
