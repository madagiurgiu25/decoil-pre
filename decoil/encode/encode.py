...
def cleanvcf(vcfin, vcfout, svcaller):
	"""
		...
		with open(vcfin, "r") as f:
			for l in f:
				if l.startswith("#"):
					...
					if svcaller in [vp.SNIFFLES1, vp.SNIFFLES2, vp.LUMPY, vp.DELLY, vp.CUTESV] and len(l.strip().split("\t")) == 10:
						g.write(l)
					elif svcaller in [vp.NANOMONSV, vp.SAVANA] and len(l.strip().split("\t")) == 11:
						g.write(l)
					else:
...
def transform_record_nanomonsv(record):
	...
	if record.INFO[vp.SVTYPE] == vp.DEL:
		record.INFO[vp.STRAND] = "+-"
	elif record.INFO[vp.SVTYPE] in [vp.DUP,vp.DUPINS,vp.INVDUP,vp.INS,vp.DUPINS]:
		record.INFO[vp.STRAND] = "-+"
	elif record.INFO[vp.SVTYPE] == vp.INV:
		record.INFO[vp.STRAND] = "--"
	elif record.INFO[vp.SVTYPE] in [vp.BND, vp.TRA]:
		record.INFO[vp.STRAND] = record.ALT[0].orientation + record.ALT[0].mate_orientation
	else:
		raise Exception("""Strand information not found {}""".format(record))
	
	record.calls[0].data[vp.DP] = int(record.calls[0].data[vp.TR])
	record.calls[0].data[vp.DV] = int(record.calls[0].data[vp.VR])
	record.calls[0].data[vp.DR] = record.calls[0].data[vp.DP] - record.calls[0].data[vp.DV]
	record.calls[0].data[vp.AF] = (record.calls[0].data[vp.DV]/(record.calls[0].data[vp.DV]+record.calls[0].data[vp.DR]+1))
...
def parsevcf(vcffile_clean, svcaller):
	...
	try:
		for record in reader:
			...
			state, process_svs = operations.pass_filter(record, processed_mates=process_svs)
...
def readvcf(vcffile, outputdir, svcaller=vp.SNIFFLES1):
	...
	cleanvcf(vcffile,vcffile_clean,svcaller)

	# parse vcf
	svinfo, collection_breakpoints, count = parsevcf(vcffile_clean,svcaller)
	# print(collection_breakpoints)

	log.info(f"Total number of entries in vcf {count}")
	log.info(f"SV kept for graph generation {len(svinfo)}")
	# sort breakpoints
