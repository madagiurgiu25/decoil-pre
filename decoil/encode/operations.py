...
def merge_near_breakpoints(collection_breakpoints, svinfo, distance=QUAL.DISTANCE):
    ...
    for chr in collection_breakpoints:
        ...
        breakpoints_todelete = (
            {}
        )  # ref_breakpoint : [list of breakpoints in close proximity]
            ...
            while i < nrbreakpoints - 1:
                ...
                while j < nrbreakpoints:
                    ...
                    if in_window(collection_breakpoints[chr], i, j, distance):
                        ...
                        breakpoints_todelete[reference_breakpoint].append(
                            close_breakpoint
                        )
...
DICT_TRANSFORM = {
    ...
    vp.NANOMONSV: transform_record_nanomonsv,
    vp.LUMPY: transform_record_lumpy,
    vp.DELLY: transform_record_delly,
}
...
def pass_filter(record, processed_mates=[], multi=False):
    ...
    try:
        ...
        print(e.args)
        sys.exit("##Exception: DR/DV has no value. Your VCF might not be genotyped. Rerun SV calling using --genotype")
...
def filter(vcf, vcfout, svcaller=vp.SNIFFLES1, multi=False):
    """
    ...
    for u in nodes:
        for v in graph.graph[u]:
            ...
            if (
                ...
                and v
                and u != v
                and nodes[u].parent_fragment != nodes[v].parent_fragment
                and len(graph.graph[u][v]) > 1
            ):
        ...
        for etemp in graph[u][v]:
            ...
            if etemp != e and edges[etemp][gep.SVTYPE] != gp.FRAGMENT:
                ...
                if edges[etemp][gep.SVTYPE] == gp.DUP:
                    print(
                        "##INFO: Found tandem duplication fragid:(tail, head, edge) {}:({},{},{}) {}".format(
                            fid, u, v, e, edges[etemp][gep.SVTYPE]
                        )
                    )
                    edges[etemp][gep.REPEAT] = True
                else:
                    print(
                        "##INFO: Found other loop type fragid:(tail, head, edge) {}:({},{},{}) {}".format(
                            fid, u, v, e, edges[etemp][gep.SVTYPE]
                        )
                    )
                    edges[etemp][gep.REPEAT] = True
    ...
    if bw.isBigWig() == True:
        ...
        for fid in fragments:
            ...
            if chr not in all_chroms:
                raise ValueError(
                    """{} extracted from vcf not present in {}. Please check formatting""".format(
                        str(chr), bigwigfile
                    )
                )
...
def add_spatial_edges_new(graph, bamfile, window=500, padd=100, fast=False):
    ...
    for i in range(len(frag_keys) - 1):
        ...
        chr1, start1, end1 = (
            fragments[fid1].chr,
            fragments[fid1].start,
            fragments[fid1].end,
        )
        chr2, start2, end2 = (
            fragments[fid2].chr,
            fragments[fid2].start,
            fragments[fid2].end,
        )
        breakpoint = -1
        ...
        if breakpoint > -1:
            ...
            if passqc(count_coverage_through, QUAL.MEAN_COVERAGE_WGS):
                ...
                if (
                    left
                    and right
                    and conn1 not in fragments_connected
                    and conn2 not in fragments_connected
                ):
                    print(
                        "Spatial ->> Chr breakpoint count node_left node_right",
                        chr1,
                        breakpoint,
                        count_coverage_through,
                        left,
                        right,
                    )
                    graph.add_edge(
                        MultiGraph.generate_id(), left, right, gp.SPATIAL, settings
                    )
