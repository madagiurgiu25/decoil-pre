...
def run_reconstruction(
    ...
    metrics.set_min_fragment_coverage(
        QUAL.MEAN_COVERAGE_WGS, QUAL.MINIMAL_FRAGMENT_COVERAGE
    )

    # 1 Graph generation
    # 1.1 Encode
    G = encode.run_encoding(
        vcffile, bigwigfile, bamfile, outputdir, multi=multi, svcaller=svcaller
    )
    ...
    likely_candidates_transformed = cycles.transform_fid2genomicpos(
        likely_candidates_filtered, G
    )
    likely_candidates_simplified = cycles.simplify_likely_candidates(
        likely_candidates_transformed
    )
...
def process_commandline_decoil_only(subparsers):
    ...
    requiredNamed.add_argument(
        "-o", "--outputdir", help="Output directory", required=True
    )
    requiredNamed.add_argument("--name", help="Name of the sample", required=True)
    requiredNamed.add_argument(
        "-r", "--reference-genome", help="Reference genome (fasta)", required=True
    )
    parser_a.add_argument(
        "-g", "--annotation-gtf", help="GTF annotation", required=False
    )
    ...
    parser_a.add_argument(
        "--multi", help="Multi-sample VCF file", action="count", default=0
    )
    parser_a.add_argument(
        "--skip",
    ...
    parser_a.add_argument(
        "--min-sv-len",
        help="Minimal SV length (default: %(default)sX)",
        required=False,
        default=QUAL.MINIMAL_SV_LEN,
        type=int,
    )
    parser_a.add_argument(
        "--fragment-min-cov",
    ...
    parser_a.add_argument(
        ...
        required=False,
        default=QUAL.MAXIMAL_FRAGMENT_COVERAGE,
        type=int,
    )
    parser_a.add_argument(
        "--fragment-min-size",
    ...
    parser_b = subparsers.add_parser(
        PROG.VALIDATE, help="Validate true against reconstruct ecDNA"
    )
    parser_b.add_argument(
        "-a", help="Configuration file for the true ecDNA", required=True
    )
    parser_b.add_argument(
        "-b", help="Configuration file for the reconstructed ecDNA", required=True
    )
    parser_b.add_argument("-o", "--outputfile", help="Output file", required=True)
    parser_b.add_argument(
        "-r", "--genome", help="Reference genome (fasta)", required=True
    )
    ...
    parser_d.add_argument(
        "-x", "--format", help="File format {VCF}", required=True, default="VCF"
    )
