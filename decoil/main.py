...
def run_save_fragments(G, outfile):
	with open(outfile, "w") as f:
		...
		for fid in fdict:
			...
			f.write(
				"""{}\t{}\t{}\t{}\t{}\n""".format(
					str(_chr), str(start), str(end), str(fid), str(int(coverage))
				)
			)
...
def run_save_v2_new(likelycandidates, G, ref_genome, outputdir, sample, skip=False):
	...
	parse.convert_path2bed_v2(
		likelycandidates, bedfile_ecdna, keep="ecDNA", score_threshold=0
	)
	parse.convert_path2bed_v2(
		likelycandidates,
		bedfile_ecdna_filtered,
		keep="ecDNA",
...
def debug_cleaning(G):
	...
	for chr_ in ["chr1", "chr2", "chr3", "chr6"]:
		...
		print(
			"After cleaning, remaining fragments vs fragments intervals",
			chr_,
			lenfrags,
			lenintervals,
		)
...
def run_reconstruction(
	...
	G = operations.remove_highcoverage_fragments(G, threshold=QUAL.MAXIMAL_FRAGMENT_COVERAGE)
...
def process_commandline_decoil_only(subparsers):
	...
	parser_a.add_argument('-b', '--bam', help='Bam file (not required when --fast active)', required=False)
	...
	parser_a.add_argument("--skip", help="Skip creation of fasta files (to save space)", action="count", default=0)
	parser_a.add_argument('--min-sv-len', help='Minimal SV length (default: %(default)sX)', required=False, default=QUAL.MINIMAL_SV_LEN, type=int)
	parser_a.add_argument('--fragment-min-cov', help='Minimal fragment coverage (default: %(default)sX)', required=False, default=QUAL.MINIMAL_FRAGMENT_COVERAGE, type=int)
	parser_a.add_argument('--fragment-max-cov', help='Maximal fragment coverage (default: %(default)sX)', required=False, default=QUAL.MAXIMAL_FRAGMENT_COVERAGE, type=int)
	parser_a.add_argument('--fragment-min-size', help='Minimal fragment size (default: %(default)sbp)', required=False, default=QUAL.MINIMAL_FRAGMENT_SIZE, type=int)
	parser_a.add_argument('--min-vaf', help='Minimal VAF acceptance SV (default: %(default)s)', required=False, default=QUAL.MIN_VAF, type=float)
	parser_a.add_argument('--min-cov-alt', help='Minimal supporting reads SV (default: %(default)sX)', required=False, default=QUAL.MIN_COV_ALT, type=int)
	parser_a.add_argument('--max-explog-threshold', help='Maximal score; better not change this (default: %(default)s)', required=False, default=QUAL.EXPLOG_THRESHOLD, type=float)
	parser_a.add_argument('--min-cov', help='Minimal coverage on site (default: %(default)sX)', required=False, default=QUAL.MIN_COV, type=int)
	parser_a.add_argument('--sv-caller', help="""SV caller name matching the VCF input {}""".format(VCF_PROP.SVCALLERS), required=False, default=VCF_PROP.SNIFFLES1)
...
def process_commandline_decoil_fullpipeline(parser, subparsers):
	...
	parser_b.add_argument('--min-sv-len', help='Minimal SV length (default: %(default)sX)', required=False, default=QUAL.MINIMAL_SV_LEN, type=int)
	parser_b.add_argument('--fragment-min-cov', help='Minimal fragment coverage (default: %(default)sX)', required=False, default=QUAL.MINIMAL_FRAGMENT_COVERAGE, type=int)
	parser_b.add_argument('--fragment-max-cov', help='Maximal fragment coverage (default: %(default)sX)', required=False, default=QUAL.MAXIMAL_FRAGMENT_COVERAGE, type=int)
	parser_b.add_argument('--fragment-min-size', help='Minimal fragment size (default: %(default)sbp)', required=False, default=QUAL.MINIMAL_FRAGMENT_SIZE, type=int)
	parser_b.add_argument('--min-vaf', help='Minimal VAF acceptance SV (default: %(default)s)', required=False, default=QUAL.MIN_VAF, type=float)
	parser_b.add_argument('--min-cov-alt', help='Minimal supporting reads SV (default: %(default)sX)', required=False, default=QUAL.MIN_COV_ALT, type=int)
	parser_b.add_argument('--max-explog-threshold', help='Maximal score; better not change this (default: %(default)s)', required=False, default=QUAL.EXPLOG_THRESHOLD, type=float)
	parser_b.add_argument('--min-cov', help='Minimal coverage on site (default: %(default)sX)', required=False, default=QUAL.MIN_COV, type=int)
	parser_b.add_argument('--filter-score', help='Filter circular structures by estimated proportions (default: %(default)d)', required=False, default=0, type=int)
	parser_b.add_argument("--filt-version", help="Sniffles2 filter version", required=False, default=1, type=int)
	...
	requiredNamed.add_argument('--name', help='Name of the sample', required=True)
	requiredNamed.add_argument('-r', '--reference-genome', help='Reference genome (fasta)', required=True)
	parser_d.add_argument('-b', '--bam', help='Bam file (not required when --fast active)', required=False)
	...
	parser_d.add_argument("--skip", help="Skip creation of fasta files (to save space)", action="count", default=0)
	parser_d.add_argument('--min-sv-len', help='Minimal SV length (default: %(default)sX)', required=False, default=QUAL.MINIMAL_SV_LEN, type=int)
	parser_d.add_argument('--fragment-min-cov', help='Minimal fragment coverage (default: %(default)sX)', required=False, default=QUAL.MINIMAL_FRAGMENT_COVERAGE, type=int)
	parser_d.add_argument('--fragment-max-cov', help='Maximal fragment coverage (default: %(default)sX)', required=False, default=QUAL.MAXIMAL_FRAGMENT_COVERAGE, type=int)
	parser_d.add_argument('--fragment-min-size', help='Minimal fragment size (default: %(default)sbp)', required=False, default=QUAL.MINIMAL_FRAGMENT_SIZE, type=int)
	parser_d.add_argument('--min-vaf', help='Minimal VAF acceptance SV (default: %(default)s)', required=False, default=QUAL.MIN_VAF, type=float)
	parser_d.add_argument('--min-cov-alt', help='Minimal supporting reads SV (default: %(default)sX)', required=False, default=QUAL.MIN_COV_ALT, type=int)
	parser_d.add_argument('--max-explog-threshold', help='Maximal score; better not change this (default: %(default)s)', required=False, default=QUAL.EXPLOG_THRESHOLD, type=float)
	parser_d.add_argument('--min-cov', help='Minimal coverage on site (default: %(default)sX)', required=False, default=QUAL.MIN_COV, type=int)
	parser_d.add_argument('--filter-score', help='Filter circular structures by estimated proportions (default: %(default)d)', required=False, default=0, type=int)
